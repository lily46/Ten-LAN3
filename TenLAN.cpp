          ...
          if ( MikanFile->Open( 0, filepath, "r" ) >= 0 )
          {
            ...
            while( MikanFile->ReadLine( 0, buf, 512 ) )
            {
              ...
              } else if (strcmp(str, "first") == 0)
              {
                ...
                if (newline < gamedata[msel].first) { newline = gamedata[msel].first; }
            ...
            // 最大ゲーム数。
            ++_loadgame;
            if ( loadgame ){ *loadgame = _loadgame; }
          }
        }
      }
...
int TenLANSystem::GetGameMax( void ){ return gamemax; }
...
int TenLANSystem::GetGameMax( unsigned int category )
{
  ...
  if ( CATEGORY_MAX <= category ){ return -1; }
...
int TenLANSystem::GetGameNumbersNotCategory( unsigned int notcategory, int *gamearray, int gamenum, int start )
{
  ...
  if ( CATEGORY_MAX <= notcategory ){ return -1; }
...
class Config * TenLANSystem::GetConfig(void) { return config; }
int TenLANSystem::GetExitTime( void ){ return config->GetExitTime(); }
int TenLANSystem::GetWidth( void ){ return width; }
int TenLANSystem::GetHeight( void ){ return height; }
int TenLANSystem::GetCategoryMax(void){ return categorymax; }
int TenLANSystem::GetCategoryGames( int catenum )
{
  if (catenum < 0 || CATEGORY_MAX <= catenum){ return 0; }
...
int TenLANSystem::GetUp( void ){ return key->GetUp(); }
int TenLANSystem::GetDown( void ){ return key->GetDown(); }
int TenLANSystem::GetLeft( void ){ return key->GetLeft(); }
int TenLANSystem::GetRight( void ){ return key->GetRight(); }

int TenLANSystem::GetY( void ){ return key->GetY(); }
int TenLANSystem::GetA( void ){ return key->GetA(); }
int TenLANSystem::GetB( void ){ return key->GetB(); }
int TenLANSystem::GetX( void ){ return key->GetX(); }
int TenLANSystem::GetL( void ){ return key->GetL(); }
int TenLANSystem::GetR( void ){ return key->GetR(); }
int TenLANSystem::GetMode( void ){ return config->GetMode(); }
